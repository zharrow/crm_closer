...
function AnimateIcon({
  ...
  /**
   * MODIFICATION LOCALE — mouvement réduit.
   *
   * Animate UI ne regarde pas `prefers-reduced-motion`, et la règle CSS de
   * `globals.css` ne peut rien pour lui : Motion anime en JavaScript, via la
   * Web Animations API, hors de portée d'un `animation-duration` en CSS.
   * Sans ce garde-fou, activer les icônes animées cassait en silence une
   * garantie que l'app tenait depuis le début.
   *
   * Tout passe par `startAnimation` — survol, appui, entrée dans le champ de
   * vision, boucle — donc un seul point d'arrêt suffit. L'icône reste
   * affichée, elle ne bouge simplement plus.
   *
   * À reporter lors d'une mise à jour des composants Animate UI.
   */
  ...
  const startAnimation = React.useCallback(
    (trigger: TriggerProp) => {
      // Voir la note ci-dessus : le seul point d'arrêt nécessaire.
    ...
    [animation, delay, bumpGeneration, prefersReducedMotion],
  );
